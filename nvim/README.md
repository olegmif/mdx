...
require("mdx").setup({
    keymaps = {
        follow = { "<leader>mf", "<CR>" },        -- открыть ссылку в текущем окне
        follow_split = { "<leader>ms", "<C-CR>" }, -- открыть в вертикальном сплите
...
`follow` и `follow_split` допускают и строку, и список строк (несколько биндингов на одно действие). Остальные keymap'ы — строкой.
